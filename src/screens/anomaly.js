    ...
    .map(
      ...
      <button type="button" class="anomaly-card" data-card-id="${c.id}">
        <div class="anomaly-card-top">
          ...
          <span class="anomaly-card-id">Last Scan ${c.scan}</span>
        </div>
        <h3>#${c.id} — ${c.title}</h3>
        <p>${c.body}</p>
        <div class="card-foot">
    ...
    <section>
      ...
      <div class="search-row">
        ...
        <div class="view-toggles" role="group" aria-label="View mode">
          <button type="button" class="toggle-btn active" title="Grid view" aria-pressed="true"><span class="toggle-btn-icon" aria-hidden="true">${trimSvg(iconViewBlock)}</span></button>
          <button type="button" class="toggle-btn" title="List view" aria-pressed="false"><span class="toggle-btn-icon" aria-hidden="true">${trimSvg(iconViewList)}</span></button>
